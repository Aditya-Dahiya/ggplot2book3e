...
Now, lets have a look at the average number of Lines of Code in the R packages released each month in an interactive heat-map in @fig-loc . We also can view the trend of average number of lines of code in R packages released over time @fig-loc2, along with a loess smoother trend-line. In the @fig-loc2, note the use of `coord_cartesian(ylim = c(0, 4000))` to zoom-in on the lower part of y-axis without removing values above 4000 from the plot.

```{r}
#| label: fig-loc
#| fig-cap: "An interactive heat-map for average number of Lines of Code in the R packages released during the month"
#| fig-width: 10
#| fig-asp: 0.6
...
Finally, the @fig-files looks at the average number of files per new R package over the years, both as a heat map in @fig-files-1, and as a line graph in @fig-files-2.

```{r}
#| label: fig-files
#| fig-cap: "Using heatmap and line graph to look at Average number of file in the /R directory of the newly released R packages"
#| fig-subcap: 
#|   - "An interactive heat-map for average number of files in the /R directory of the R packages released during the month"
#|   - "A line-graph for average number of files in the /R directory of the newly released R packages over time"
#| layout-ncol: 1
#| fig-width: 10
...
g3 <- df2 |> 
  ...
  scale_x_continuous(expand = expansion(0)) + 
  scale_y_discrete(expand = expansion(0)) +
  labs(title = "Average Number of files in /R code of newly released R packages",
       x = NULL, y = NULL,
       fill = "Average number of Files for the packages released in the month") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    legend.key.width = unit(15, "mm"),
...
df2 |> 
  ...
  geom_point(col = "black", 
             fill = "white",
             pch = 1) +
  labs(title = "Average number of files in /R of new R packages",
...
Now, let us explore the kind of licenses used by newly released R packages over the years. The @fig-license shows the share of different licenses in an interactive stacked bar chart, while @fig-license1 shows different characteristics of packages with these licenses: —

```{r}
#| label: fig-license
#| fig-cap: "Exploring the licenses of R packages"
#| fig-width: 10
#| fig-asp: 0.6
...
g4 <- rpkgstats |> 
  ...
  geom_col_interactive(aes(
    tooltip = paste0("Year: ", year,
                     "\nTotal Number of packages released: ", yeartotal,
  ...
  labs(y = "% Packages with a particular license",
       x = "Year",
       fill = "License",
       title = "Share of license types on new R packages over time",
...
Instead of the stacked bar-plot, we can also make a stream graph using `ggstream` [@ggstream] to make the graph more visually appealing, at the cost of losing some detailed numbers etc. As shown in the stream graph in @fig-ggstr, it gives a better overall view: —

```{r}
#| label: fig-ggstr
#| fig-cap: "Visualizing the change in dominance of different licenses over time with a stream graph"
#| fig-width: 10
#| fig-asp: 0.8
...
rpkgstats |> 
  ...
  labs(
    x = NULL, y = NULL,
    title = "Dominance of different licenses in R Packages released over the years",
    subtitle = "MIT + file License is an increasingly preferred choice in recent years",
    caption = "Created using {ggstream} and {paletteer}"
  ) +
  scale_x_continuous(expand = expansion(0)) +
