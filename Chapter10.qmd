...
rpkgstats |> 
  ...
  labs(
    x = NULL, y = NULL,
    title = "Dominance of different licenses in R Packages",
    subtitle = "MIT + file License is an increasingly preferred choice in recent years",
    caption = "Created using {ggplot2}, {ggstream} and {paletteer}"
  ) +
  scale_x_continuous(expand = expansion(0),
...
To demonstrate the `log10` scale on the x-axis, we use an animated histogram depicting the distribution of the Lines of Code in R packages over the years using {scales} [@scales] and {gganimate} [@gganimate-3], as depicted in @fig-anim1
...
rpkgstats |> 
  mutate(timeperiod = date > as_date("2020-01-01"),
         year = year(date)) |> 
  ggplot(aes(loc_R)) +
  geom_histogram(bins = 50, col = "#940c00", fill = "#e8756b") +
  ...
  scale_y_continuous(expand = expansion(0)) +
  labs(
    x = "Number of lines of Code in the Package (on the Log scale)",
    y = "Number of R packages",
    title = "Change in distribution of Lines of Code in R packages over the years"
  ) +
