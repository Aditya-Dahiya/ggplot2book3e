...
Now, lets have a look at the average number of Lines of Code in the R packages over time @fig-loc and its trend month-wise in an interactive heat-map in @fig-loc2. In the @fig-loc2, note the use of `coord_cartesian(ylim = c(0, 4000))` to zoom-in on the lower part of y-axis without removing values above 4000 from the plot.
...
#| fig-subcap:
#|   - "A Stacked bar-chart of the proportion of different license types of the years"
#|   - "Interactive Version of the same stacked bar chart"
...
### A look at the "tidy" packages in R
...
rpkgstats |> 
  ...
  ggplot(aes(x = date, 
             ...
             label = package,
             size = files_R)) +
  geom_text_interactive(alpha = 0.5) +
  theme_clean()
```
